...
class WebhookPreferStrategy(RoutingStrategy):
    """Prefer models with webhook capability when requested.

    When ``metadata.requires_webhook``, ``metadata.webhook_callbacks``, or
    ``metadata.async_webhook`` is truthy, rank healthy domain-eligible
    candidates by whether they support ``webhook``, then by quality
    (descending) and cost (ascending). Capability is resolved from
    ``metadata.webhook_models``, ``metadata.model_capabilities`` / the
    built-in known-model map (``webhook`` capability), or a name
    heuristic matching ``webhook``. Requests that omit the signal stay
    quality-first. Inspired by OpenAI / Anthropic async webhook callbacks
    and Gemini long-running job notifications on GPT-5.5 / Claude Sonnet 4.6 /
    Gemini 3.x / Kimi K2.
    """

    strategy_name = RoutingStrategyName.WEBHOOK_PREFER
    ...
    def __init__(
        ...
        """Initialize webhook-prefer routing."""
        super().__init__(model_catalog)
        self._provider_health = provider_health
        self._capability_map: Mapping[str, frozenset[str]] = (
            _KNOWN_MODEL_CAPABILITIES if capability_map is None else capability_map
    ...
    @classmethod
    def _wants_webhook(cls, request: RouterRequest) -> bool:
        """Return whether the request asks for webhook support."""
        return (
            cls._is_truthy(request.metadata.get("requires_webhook"))
            or cls._is_truthy(request.metadata.get("webhook_callbacks"))
            or cls._is_truthy(request.metadata.get("async_webhook"))
        )

    @staticmethod
    def _webhook_allowlist(request: RouterRequest) -> frozenset[str] | None:
        """Parse an optional ``metadata.webhook_models`` allowlist."""
        raw = request.metadata.get("webhook_models")
        if raw is None:
            return None
        if isinstance(raw, str):
            parts: Iterable[object] = raw.split(",")
    ...
    def _supports_webhook(self, model: str, request: RouterRequest) -> bool:
        """Return whether a model is treated as webhook capable."""
        allowlist = self._webhook_allowlist(request)
        if allowlist is not None:
            return model.lower() in allowlist
        capabilities = self._capabilities_for(model, request)
        if capabilities is not None:
            return "webhook" in capabilities
        lower = model.lower()
        return any(token in lower for token in self._NAME_TOKENS)

    def choose(self, request: RouterRequest, signals: TaskSignals) -> RoutingDecision:
        """Prefer webhook models when the capability is requested."""
        wants = self._wants_webhook(request)
        eligible = [
            candidate
            for candidate in self._model_catalog.values()
            if signals.domain_tag in candidate.supports_domains
        ...
        webhook_flags = {
            candidate.model: self._supports_webhook(candidate.model, request)
            for candidate in active
        }
        availability_note = "healthy" if healthy else "circuit-open emergency"
        ...
        if wants:
            ...
            fallback_candidates = sorted(
                (candidate for candidate in active if candidate.model != selected.model),
                key=lambda candidate: (
                    not webhook_flags[candidate.model],
                    -candidate.quality_score,
                    costs[candidate.model],
                    candidate.model,
                ),
            ...
            capable_note = (
                "webhook-capable" if webhook_flags[selected.model] else "non-webhook fallback"
            )
            rationale = (
                f"webhook-prefer requested; selected {availability_note} "
                f"{capable_note} {selected.model} (quality {selected.quality_score:.2f})"
            )
        else:
            selected = max(
...
class MemoryToolPreferStrategy(RoutingStrategy):
    """Prefer models with memory_tool capability when requested.

    When ``metadata.requires_memory_tool``, ``metadata.memory_tool``, or
    ``metadata.agent_memory`` is truthy, rank healthy domain-eligible
    candidates by whether they support ``memory_tool``, then by quality
    (descending) and cost (ascending). Capability is resolved from
    ``metadata.memory_tool_models``, ``metadata.model_capabilities`` / the
    built-in known-model map (``memory_tool`` capability), or a name
    heuristic matching ``memory_tool``. Requests that omit the signal stay
    quality-first. Inspired by Anthropic memory tool / OpenAI Agents API persistent memory
    gaps on GPT-5.5 / Claude Sonnet 4.6 /
    Gemini 3.x / Kimi K2.
    """

    strategy_name = RoutingStrategyName.MEMORY_TOOL_PREFER
    ...
    def __init__(
        ...
        """Initialize memory-tool-prefer routing."""
        super().__init__(model_catalog)
        self._provider_health = provider_health
        self._capability_map: Mapping[str, frozenset[str]] = (
            _KNOWN_MODEL_CAPABILITIES if capability_map is None else capability_map
    ...
    @classmethod
    def _wants_memory_tool(cls, request: RouterRequest) -> bool:
        """Return whether the request asks for memory_tool support."""
        return (
            cls._is_truthy(request.metadata.get("requires_memory_tool"))
            or cls._is_truthy(request.metadata.get("memory_tool"))
            or cls._is_truthy(request.metadata.get("agent_memory"))
        )

    @staticmethod
    def _memory_tool_allowlist(request: RouterRequest) -> frozenset[str] | None:
        """Parse an optional ``metadata.memory_tool_models`` allowlist."""
        raw = request.metadata.get("memory_tool_models")
        if raw is None:
            return None
        if isinstance(raw, str):
            parts: Iterable[object] = raw.split(",")
    ...
    def _supports_memory_tool(self, model: str, request: RouterRequest) -> bool:
        """Return whether a model is treated as memory_tool capable."""
        allowlist = self._memory_tool_allowlist(request)
        if allowlist is not None:
            return model.lower() in allowlist
        capabilities = self._capabilities_for(model, request)
        if capabilities is not None:
            return "memory_tool" in capabilities
        lower = model.lower()
        return any(token in lower for token in self._NAME_TOKENS)

    def choose(self, request: RouterRequest, signals: TaskSignals) -> RoutingDecision:
        """Prefer memory_tool models when the capability is requested."""
        wants = self._wants_memory_tool(request)
        eligible = [
            candidate
            for candidate in self._model_catalog.values()
            if signals.domain_tag in candidate.supports_domains
        ...
        memory_tool_flags = {
            candidate.model: self._supports_memory_tool(candidate.model, request)
            for candidate in active
        }
        availability_note = "healthy" if healthy else "circuit-open emergency"
        ...
        if wants:
            selected = max(
                active,
                key=lambda candidate: (
                    memory_tool_flags[candidate.model],
                    candidate.quality_score,
                    -costs[candidate.model],
                    candidate.model,
                ),
            ...
            fallback_candidates = sorted(
                (candidate for candidate in active if candidate.model != selected.model),
                key=lambda candidate: (
                    not memory_tool_flags[candidate.model],
                    -candidate.quality_score,
                    costs[candidate.model],
                    candidate.model,
                ),
            ...
            capable_note = (
                "memory_tool-capable"
                if memory_tool_flags[selected.model]
                else "non-memory_tool fallback"
            )
            rationale = (
                f"memory-tool-prefer requested; selected {availability_note} "
                f"{capable_note} {selected.model} (quality {selected.quality_score:.2f})"
            )
        else:
            selected = max(
            ...
            rationale = (
                f"memory-tool-prefer no signal; "
                f"selected {availability_note} quality-first {selected.model}"
            )
...
def build_strategies(
    ...
    return {
        ...
        RoutingStrategyName.WEBHOOK_PREFER: WebhookPreferStrategy(
            model_catalog,
            provider_health,
        ),
        RoutingStrategyName.MEMORY_TOOL_PREFER: MemoryToolPreferStrategy(
            model_catalog,
            provider_health,
        ),
