...
def test_assert_allowed_consumes_tokens_then_raises(monkeypatch: pytest.MonkeyPatch) -> None:
...
def test_tenants_are_isolated(monkeypatch: pytest.MonkeyPatch) -> None:
...
def test_distinct_from_api_key_limiter_import(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(time, "monotonic", lambda: 25.0)
