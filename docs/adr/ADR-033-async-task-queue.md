# ADR-033: Async Task Queue for nexus-llm-router
...
that integrates cleanly with our async adapter pipeline.
...
- All new adapter components will use `Redis Streams` as the async task queue layer.
